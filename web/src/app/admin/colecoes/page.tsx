...
export default function AdminCollectionsPage() {
  ...
  const load = React.useCallback(() => {
    fetch("/collections").then((r) => r.json()).then(setList).catch(() => setList([]));
  }, []);
  React.useEffect(() => { load(); }, [load]);

  async function add(e: React.FormEvent) {
    e.preventDefault();
    await fetch("/collections", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, slug }) });
    setName(""); setSlug("");
    load();
  }
  ...
  return (
    <div>
      <div className="heading-3">Coleções</div>
